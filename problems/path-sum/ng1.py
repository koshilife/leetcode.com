...
# NG reason:
# I overlooked the below meaning.
    ...
    def hasPathSum(self, root: Optional[TreeNode], target_sum: int) -> bool:
        def search(_node, current_sum, depth=0):
            ...
            if current_sum == target_sum:
                is_leaf = depth > 0 and (not _node.left or not _node.right)
                is_root_has_no_leaf = depth == 0 and not _node.left and not _node.right
                if is_leaf:
                    return True
                if is_root_has_no_leaf:
                    return True
